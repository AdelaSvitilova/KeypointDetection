import numpy as np
from pycocotools.coco import COCO
import cv2
import os
import csv
from .base_dataset import BaseDataset

class COCODataset(BaseDataset):
    def __init__(self, root_dir, load, transform=None, num_keypoints=17):
        ann_file = os.path.join(root_dir, 'annotations.json')
        img_dir = os.path.join(root_dir, 'images')
        load_file = os.path.join(root_dir, load)

        self.coco = COCO(ann_file)
        self.img_dir = img_dir
        self.transform = transform
        self.num_keypoints = num_keypoints

        # 1) načíst jména souborů
        with open(load_file, newline='') as f:
            file_names = [row[0].strip() for row in csv.reader(f) if row]

        # 2) mapování
        name_to_id = {info["file_name"]: img_id for img_id, info in self.coco.imgs.items()}

        # 3) připrav pole itemů
        self.items = []
        for fname in file_names:
            if fname not in name_to_id:
                continue

            img_id = name_to_id[fname]

            # načti anotaci:
            ann_ids = self.coco.getAnnIds(imgIds=img_id, iscrowd=False)
            anns = self.coco.loadAnns(ann_ids)

            keypoints = np.zeros((num_keypoints, 3), dtype=np.float32)

            if anns and "keypoints" in anns[0]:
                raw = anns[0]["keypoints"]
                if len(raw) == num_keypoints * 3:
                    keypoints = np.array(raw, dtype=np.float32).reshape(num_keypoints, 3)

            self.items.append({
                "file_name": fname,
                "keypoints": keypoints
            })

        print(load, [item["file_name"] for item in self.items])


    def __len__(self):
        return len(self.items)

    def __getitem__(self, idx):
        item = self.items[idx]

        # načti obrázek
        path = os.path.join(self.img_dir, item["file_name"])
        image = cv2.imread(path)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB) / 255.0

        orig_h, orig_w = image.shape[:2]
        target_size = (256, 256)

        # resize
        image = cv2.resize(image, target_size)

        # scale pro keypointy
        sx = target_size[0] / orig_w
        sy = target_size[1] / orig_h

        keypoints = item["keypoints"].copy()
        keypoints[:, 0] *= sx
        keypoints[:, 1] *= sy

        # CHW
        image = image.transpose(2, 0, 1)

        if self.transform:
            image, keypoints = self.transform(image, keypoints)

        return image, keypoints
